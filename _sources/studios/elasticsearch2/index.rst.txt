...
#. Fix the issue with ``location`` by editing tweets_geo.sh to explicitly map ``location`` as a ``geo_point`` field. **Hint**: To build the JSON that you'll need to create the new mapping for ``twitter_geo``, you can copy the response from fetching the mapping for ``twitter`` and modify it.
...
#. Find all tweets that contain the text "Elasticsearch", including misspellings up to distance of 2 away.
