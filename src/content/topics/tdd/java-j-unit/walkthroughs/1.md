...
After adding the code, run the tests again. Our tests should now run **and they shouldl fail**.
...
We need to add a property to `tripMeter` and then update th emethods we just added: `getTripMeter()` and `resetTripMeter()` to `Car.java`.
