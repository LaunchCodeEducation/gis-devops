...
- **independent**: tests should not rely on nor effect the testing state of other tests
...
> All tests perform a `GET /notes` request interaction
...
| **populated** | insert row(s) | **status**: 200, **body**: `Note[]`  | - | remove row(s) |
...
> All tests perform a `POST /notes` request interaction
...
# assert the table is empty (to later confirm it increased by 1)
...
# assert that the new note has an ID property assigned to it
